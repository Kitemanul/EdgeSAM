...
def fix_dtypes_for_npu(onnx_path):
    """Fix int16 and selective int64 → int32 for NPU compatibility.

    - Cast→int16 (from stability score .sum(dtype=torch.int16)) → int32
    - Cast→int64 (from index_select) → int32, UNLESS the output feeds
      an op that requires int64 (Reshape shape, Slice indices, etc.)
    - Same logic for Constant nodes and Initializers.
    """
    import onnx
    ...
    for node in model.graph.node:
        ...
        for attr in node.attribute:
            if attr.name != "to" or attr.i not in convertible:
                continue
            # int16 → always convert
            if attr.i == INT16:
                attr.i = INT32
                count += 1
            # int64 → convert only if output is NOT required to stay int64
            elif attr.i == INT64:
    ...
    print(f"  Fixed {count} dtype occurrences (int16/int64 → int32, preserving Reshape/Slice/etc. shapes)")
