...
# ============================================================
# ONNX post-processing: convert int16 → int32 for NPU
# ============================================================

def fix_int16_for_npu(onnx_path):
    """Convert int16 → int32 in the ONNX model. Leave int64 untouched.

    The only problematic type is int16, generated by
    calculate_stability_score's .sum(dtype=torch.int16).
    int64 must stay as-is for ONNX spec compliance (Reshape, Tile, etc.).
    """
    import onnx
    ...
    # Cast nodes targeting int16
    for node in model.graph.node:
        if node.op_type == "Cast":
            for attr in node.attribute:
                if attr.name == "to" and attr.i == INT16:
                    attr.i = INT32
                    count += 1

    # Constant nodes with int16 data
    for node in model.graph.node:
        if node.op_type == "Constant":
            for attr in node.attribute:
                if attr.name == "value" and attr.t and attr.t.data_type == INT16:
    ...
    for vi in list(model.graph.input) + list(model.graph.output) + list(model.graph.value_info):
        if vi.type.tensor_type.elem_type == INT16:
            vi.type.tensor_type.elem_type = INT32
            count += 1

    onnx.save(model, onnx_path)
    print(f"  Converted {count} int16 → int32 occurrences")
...
def export_encoder_to_onnx(sam, args):
    ...
    fix_int16_for_npu(onnx_encoder_filename)
...
def export_decoder_to_onnx(sam, args):
    ...
    fix_int16_for_npu(onnx_decoder_filename)
